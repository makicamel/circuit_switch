...
class ReporterTest < Test::Unit::TestCase
  ...
  def test_reporter_calls_config_reporter
    message = CircuitSwitch::Reporter.new.perform(limit_count: 10, called_path: 'test')
    assert_equal(
  ...
  def test_reporter_creates_circuit_switch_when_no_switch
    ...
    CircuitSwitch::Reporter.new.perform(limit_count: 10, called_path: 'test')
    assert_equal(
      true,
  ...
  def test_reporter_updates_circuit_switch_report_count_when_switch_exists
    ...
    assert_equal(
      1,
      CircuitSwitch::CircuitSwitch.find_by(caller: 'test').report_count
    )
    CircuitSwitch::Reporter.new.perform(limit_count: 10, called_path: 'test')
    assert_equal(
      2,
